...
[Zustand](https://github.com/pmndrs/zustand) is one of those _new_ state management libraries. Invented well after the rise and pleatu of redux, invented well after the easification of [Context](https://reactjs.org/docs/context.html), invented in a time where there new ideas coming along all the time.

Tree local state is what I used to think Context. While it _could_ be for tree local state, it has poor performance characteristics and a developer experience problem.
...
What if you wanted some state that's shared by a group of components? Since React represents everything internally as a tree, maybe you could extend our local component state and say this higher level state, this shared state, is tree local state.

Zustand does that!

One of my favorite features of Zustand is the decoupling between it and React. With redux, you can weasal your way out of having to do everything inside of React computers. That's great. With Context, on the other hand, it is a lot more difficult.
