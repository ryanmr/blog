...
In HTML spaces collapse, so we never had an issue previously. Cloudflare saves bytes by minifying this HTML before its sent down to the browser. Great, but unfortunately Next noticed during hydration.
